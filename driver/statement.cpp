    ...
    : ChildType(connection)
{
...
#if !defined(WORKAROUND_DISABLE_SSL)
    const auto is_ssl = (Poco::UTF8::icompare(conn.proto, "https") == 0);
    statement_session = (
    ...
    statement_session->setHost(conn.server);
    statement_session->setPort(conn.port);
    statement_session->setKeepAlive(true);
    statement_session->setTimeout(
        Poco::Timespan(conn.connection_timeout, 0), 
        Poco::Timespan(conn.timeout, 0), 
        Poco::Timespan(conn.timeout, 0)
    );
